export interface IAnalisiCostoArticoliPrivate {
  _id: string;
  /** Reflected: IdAnalisiCosto*/ 
  __Dati_Testo001: string;
  /** Reflected: DataCreazione*/ 
  __Dati_Data001: Date;
  /** Reflected: DataModifica*/ 
  __Dati_Data002: Date;
  /** Reflected: DataOraCreazione*/ 
  __Dati_DataOra001: Date;
  /** Reflected: DataOraUltimaModifica*/ 
  __Dati_DataOra002: Date;
  /** Reflected: FlagAggiornaTotali01*/ 
  __Dati_Numero001: number;
  /** Reflected: FlagAggiornaTotali02*/ 
  __Dati_Numero002: number;
  /** Reflected: IdTipoArticolo*/ 
  __Dati_Testo002: string;
  /** Reflected: IdAccountCompagnia*/ 
  __Dati_Testo003: string;
  /** Reflected: Uno*/ 
  __Dati_Numero003: number;
  /** Reflected: Zero*/ 
  __Dati_Numero004: number;
  /** Reflected: PercRicarico*/ 
  __Dati_Numero005: number;
  /** Reflected: CodiceDisegno*/ 
  __Dati_Testo004: string;
  /** Reflected: RevisioneDisegno*/ 
  __Dati_Testo005: string;
  /** Reflected: IdAccountCliente*/ 
  __Dati_Testo006: string;
  /** Reflected: CodiceProgetto*/ 
  __Dati_Testo007: string;
  /** Reflected: Note*/ 
  __Dati_Testo008: string;
  /** Reflected: TxtAnalisiCosto*/ 
  __Dati_Testo009: string;
  /** Reflected: UtenteCreazione*/ 
  __Dati_Testo010: string;
  /** Reflected: UtenteModifica*/ 
  __Dati_Testo011: string;
  /** Reflected: FlagInCorso*/ 
  __Dati_Numero006: number;
  /** Reflected: FlagChiusa*/ 
  __Dati_Numero007: number;
  /** Reflected: DataOraChiusura*/ 
  __Dati_DataOra003: Date;
  /** Reflected: UtenteChiusura*/ 
  __Dati_Testo012: string;
  /** Reflected: Quantita*/ 
  __Dati_Numero008: number;
  /** Reflected: Quantita_TollPiuPerc*/ 
  __Dati_Numero009: number;
  /** Reflected: Quantita_TollMenoPerc*/ 
  __Dati_Numero010: number;
  /** Reflected: Quantita_LimiteInferiore*/ 
  __Dati_Numero011: number;
  /** Reflected: Quantita_LimiteSuperiore*/ 
  __Dati_Numero012: number;
  /** Reflected: CTADT*/ 
  __Dati_Numero013: number;
  /** Reflected: CTIngT*/ 
  __Dati_Numero014: number;
  /** Reflected: CTLET*/ 
  __Dati_Numero015: number;
  /** Reflected: CTLT*/ 
  __Dati_Numero016: number;
  /** Reflected: CTMT*/ 
  __Dati_Numero017: number;
  /** Reflected: CTProgT*/ 
  __Dati_Numero018: number;
  /** Reflected: CTPT*/ 
  __Dati_Numero019: number;
  /** Reflected: CTQT*/ 
  __Dati_Numero020: number;
  /** Reflected: CTST*/ 
  __Dati_Numero021: number;
  /** Reflected: CTT*/ 
  __Dati_Numero022: number;
  /** Reflected: CTT_Ricarico*/ 
  __Dati_Numero023: number;
  /** Reflected: CTT_UnitaBuone*/ 
  __Dati_Numero024: number;
  /** Reflected: CTTRAT*/ 
  __Dati_Numero025: number;
  /** Reflected: CTUtT*/ 
  __Dati_Numero026: number;
  /** Reflected: CUADT*/ 
  __Dati_Numero027: number;
  /** Reflected: CUIngT*/ 
  __Dati_Numero028: number;
  /** Reflected: CULET*/ 
  __Dati_Numero029: number;
  /** Reflected: CULT*/ 
  __Dati_Numero030: number;
  /** Reflected: CUMT*/ 
  __Dati_Numero031: number;
  /** Reflected: CUProgT*/ 
  __Dati_Numero032: number;
  /** Reflected: CUPT*/ 
  __Dati_Numero033: number;
  /** Reflected: CUQT*/ 
  __Dati_Numero034: number;
  /** Reflected: CUST*/ 
  __Dati_Numero035: number;
  /** Reflected: CUT*/ 
  __Dati_Numero036: number;
  /** Reflected: CUT_Ricarico*/ 
  __Dati_Numero037: number;
  /** Reflected: CUT_UnitaBuone*/ 
  __Dati_Numero038: number;
  /** Reflected: CUTRAT*/ 
  __Dati_Numero039: number;
  /** Reflected: CUUtT*/ 
  __Dati_Numero040: number;
  /** Reflected: Risultato_CUADT_Media*/ 
  __Dati_Numero041: number;
  /** Reflected: Risultato_CUADT_Deviazione*/ 
  __Dati_Numero042: number;
  /** Reflected: Risultato_CUPT_Deviazione*/ 
  __Dati_Numero043: number;
  /** Reflected: Risultato_CUPT_Media*/ 
  __Dati_Numero044: number;
  /** Reflected: Risultato_CULET_Media*/ 
  __Dati_Numero045: number;
  /** Reflected: Risultato_CULT_Media*/ 
  __Dati_Numero046: number;
  /** Reflected: Risultato_CUMT_Media*/ 
  __Dati_Numero047: number;
  /** Reflected: Risultato_CUST_Media*/ 
  __Dati_Numero048: number;
  /** Reflected: Risultato_CUTRAT_Media*/ 
  __Dati_Numero049: number;
  /** Reflected: Risultato_CUQT_Media*/ 
  __Dati_Numero050: number;
  /** Reflected: Risultato_CUUtT_Media*/ 
  __Dati_Numero051: number;
  /** Reflected: Risultato_CTIngT_Media*/ 
  __Dati_Numero052: number;
  /** Reflected: Risultato_CTProgT_Media*/ 
  __Dati_Numero053: number;
  /** Reflected: Risultato_CUT_Media*/ 
  __Dati_Numero054: number;
  /** Reflected: Risultato_CTIngT_Deviazione*/ 
  __Dati_Numero055: number;
  /** Reflected: Risultato_CULET_Deviazione*/ 
  __Dati_Numero056: number;
  /** Reflected: Risultato_CULT_Deviazione*/ 
  __Dati_Numero057: number;
  /** Reflected: Risultato_CUMT_Deviazione*/ 
  __Dati_Numero058: number;
  /** Reflected: Risultato_CTProgT_Deviazione*/ 
  __Dati_Numero059: number;
  /** Reflected: Risultato_CUQT_Deviazione*/ 
  __Dati_Numero060: number;
  /** Reflected: Risultato_CUST_Deviazione*/ 
  __Dati_Numero061: number;
  /** Reflected: Risultato_CUT_Deviazione*/ 
  __Dati_Numero062: number;
  /** Reflected: Risultato_CUTRAT_Deviazione*/ 
  __Dati_Numero063: number;
  /** Reflected: Risultato_CUUtT_Deviazione*/ 
  __Dati_Numero064: number;
  /** Reflected: IdAccountOperatore*/ 
  __Dati_Testo013: string;
  /** Reflected: TxtAccountClienteFinale*/ 
  __Dati_Testo014: string;
  /** Reflected: TxtAccountOperatore*/ 
  __Dati_Testo015: string;
  /** Reflected: DataScadenza*/ 
  __Dati_Data003: Date;
  /** Reflected: IdDocumentoDisegno*/ 
  __Dati_Testo016: string;
  /** Reflected: IdDocumentoImmagine*/ 
  __Dati_Testo017: string;
  /** Reflected: CUADT_Deviazione*/ 
  __Dati_Numero065: number;
  /** Reflected: CUIngT_Deviazione*/ 
  __Dati_Numero066: number;
  /** Reflected: CULET_Deviazione*/ 
  __Dati_Numero067: number;
  /** Reflected: CULT_Deviazione*/ 
  __Dati_Numero068: number;
  /** Reflected: CUMT_Deviazione*/ 
  __Dati_Numero069: number;
  /** Reflected: CUProgT_Deviazione*/ 
  __Dati_Numero070: number;
  /** Reflected: CUPT_Deviazione*/ 
  __Dati_Numero071: number;
  /** Reflected: CUQT_Deviazione*/ 
  __Dati_Numero072: number;
  /** Reflected: CUST_Deviazione*/ 
  __Dati_Numero073: number;
  /** Reflected: CUTRAT_Deviazione*/ 
  __Dati_Numero074: number;
  /** Reflected: CUUtT_Deviazione*/ 
  __Dati_Numero075: number;
  /** Reflected: CodificaClienteItem*/ 
  __Dati_Testo018: string;
  /** Reflected: DescrizioneClienteItem*/ 
  __Dati_Testo019: string;
  /** Reflected: TxtTipoArticolo*/ 
  __Dati_Testo020: string;
  /** Reflected: CUT_Deviazione*/ 
  __Dati_Numero076: number;
  /** Reflected: CUT_UnitaBuone_Deviazione*/ 
  __Dati_Numero077: number;
  /** Reflected: CUT_Ricarico_Deviazione*/ 
  __Dati_Numero078: number;
  /** Reflected: IdFamigliaItem*/ 
  __Dati_Testo021: string;
  /** Reflected: IdSottoFamigliaItem*/ 
  __Dati_Testo022: string;
  /** Reflected: CUADT_Id*/ 
  __Dati_Testo023: string;
  /** Reflected: CUIngT_Id*/ 
  __Dati_Testo024: string;
  /** Reflected: CULET_Id*/ 
  __Dati_Testo025: string;
  /** Reflected: CULT_Id*/ 
  __Dati_Testo026: string;
  /** Reflected: CUMT_Id*/ 
  __Dati_Testo027: string;
  /** Reflected: CUProgT_Id*/ 
  __Dati_Testo028: string;
  /** Reflected: CUPT_Id*/ 
  __Dati_Testo029: string;
  /** Reflected: CUQT_Id*/ 
  __Dati_Testo030: string;
  /** Reflected: CUST_Id*/ 
  __Dati_Testo031: string;
  /** Reflected: CUT_Id*/ 
  __Dati_Testo032: string;
  /** Reflected: CUTRAT_Id*/ 
  __Dati_Testo033: string;
  /** Reflected: CUUtT_Id*/ 
  __Dati_Testo034: string;
  /** Reflected: IdTipoCiclo*/ 
  __Dati_Testo035: string;
  /** Reflected: IdInstallazione*/ 
  __Dati_Testo036: string;
  /** Reflected: IdUtente*/ 
  __Dati_Testo037: string;
  /** Reflected: TxtUtente*/ 
  __Dati_Testo038: string;
  /** Reflected: IdRicorrenzaRecord*/ 
  __Dati_Testo039: string
}